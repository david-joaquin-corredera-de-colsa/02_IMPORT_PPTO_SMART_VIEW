...
Public Function SmartView_CreateConnection() As Boolean
    ...
    'Verificar si la conexion existe
      If vExisteConexion_Return Then
         MsgBox ("La conexion ya existe. Vamos a actualizarla.")
      Else
        MsgBox "La conexion NO existe. Vamos a crearla." & vbCrLf & "Error Number = " & vExisteConexion_Return
      End If
    ...
    'Verificar si se desconecto de todas las aplicaciones
      If vDesconectarTodo_Return = 0 Then
         MsgBox ("Se desconecto correctamente de todas las aplicaciones")
      Else
        MsgBox "No consiguio desconectarse correctamente de todas las aplicaciones" & vbCrLf & "Error Number = " & vDesconectarTodo_Return
      End If
    ...
    'Verificar si la conexion se ha eliminado
      If vEliminarConexion_Return = 0 And vExisteConexion_Return Then
         MsgBox ("Se eliminó la conexion " & CONST_FRIENDLY_NAME & " para volver a crearla con valores actualizados")
      ElseIf vExisteConexion_Return Then
        MsgBox "Hubo un error al intentar eliminar la conexión " & CONST_FRIENDLY_NAME & _
            " para volver a crearla con valores actualizados." & vbCrLf & "Error Number = " & vEliminarConexion_Return
      Else
        MsgBox "No existia la conexión " & CONST_FRIENDLY_NAME & ". " & vbCrLf & "Asi que no hubo que eliminarla." & vbCrLf & _
    ...
    'Verificar si la conexion se ha creado
      If vCrearConexion_Return = 0 Then
         MsgBox ("Se creo la conexion" & CONST_FRIENDLY_NAME & " correctamente, con valores actualizados")
      Else
         MsgBox "Fallo la creacion de la conexion" & CONST_FRIENDLY_NAME & "." & vbCrLf & "Error Number = " & vCrearConexion_Return
      End If
    ...
    'Verificar si nos hemos conectado
      If vConectar_Return = 0 Then
         MsgBox ("Nos hemos conectado correctamente a " & CONST_FRIENDLY_NAME)
      Else
         MsgBox "Fallo al conectarnos." & vbCrLf & "Error Number = " & vConectar_Return
      End If
